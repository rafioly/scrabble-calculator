...
class ScrabbleCalculatorIntegrationTest {
    ...
    void postWord_savesAndReturnsLeaderboardEntry() {
        ...
        assertEquals("hello", entries.get(0).getWord());
        assertEquals(8, entries.get(0).getScore());
    }
    ...
    void getLeaderboard_returnsSortedEntries() {
        ...
        assertEquals("cabbage", leaderboard.get(0).word());
        assertEquals(Optional.of(1), leaderboard.get(0).rank());
    }
    ...
    void getScoringRules_returnsMap() {
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertTrue(Objects.requireNonNull(response.getBody()).getData().containsKey('A'));
    }

    // Tests for GET /api/leaderboards/{id} have been removed.
}
